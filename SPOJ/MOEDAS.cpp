...
vector<vector<int>> mem(101, vector<int>(50001, -1));
vector<int> moedas;

int moeda(int i, int val){
    if(val<0) return inf;
    if(mem[i][val] == -1){
        mem[i][val] = m(1 + moeda(i, val-moedas[i]), moeda(i+1, val));
    }
    return mem[i][val];
}

void iniciar(int num_moedas, int valor_max){
