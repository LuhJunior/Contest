...
#define nwhile(n) while(cin>>n && n != 0)
...
#define mp make_pair
///*Arrumar
#define st(I) sort(I.begin(), I.end())
...
public:
    ...
    bool bo(char op){
        if(op == '+') return (this->a + this->b) == this->c;
        else if(op == '-') return (this->a - this->b) == this->c;
        else if(op == '*') return (this->a * this->b) == this->c;
        else if(op == 'I') return (((this->a + this->b) != this->c) && ((this->a * this->b) != this->c) && ((this->a * this->b) != this->c));
        return false;
    }
    lli a, b, c;
};

bool f(pair<string, bool> x){return x.second == true;}

bool cp(pair<string, bool> x, pair<string, bool> y){return x.first<y.first;}

int main() {
    int n;
    while(cin>>n){
        vector<T> v;
        vector<pair<string, bool>> nomes;
        fori(n){
            lli a, b, c;
        ...
        fori(n){
            ...
            char op;
            cin>>s>>ind>>op;
            nomes.pb(mp(s, v[ind-1].bo(op)));
        }
        int p = cifs(nomes, f);
        if(p == 0) c("None Shall Pass!");
        else if(p == n) c("You Shall All Pass!");
        else{
            sort(nomes.begin(), nomes.end(), cp);
            fori(nomes.size())
                if(!nomes[i].second){
                    if(i == nomes.size()) cout<<nomes[i].first;
                    else cout<<nomes[i].first<<" ";
                }
            c("");
        }
    }
